...
if (typeof process.env.STATUSVIEW !== 'undefined' && process.env.STATUSVIEW.toLowerCase() === 'true') { if (m.key.remoteJid === 'status@broadcast') { await conn.readMessages([m.key]); const prince = ['😀', '😃', '😄', '😁', '😊', '😇', '🙂', '🙃', '😍', '🥰', '😘', '😗', '😙', '😚', '😋', '🤑', '💌', '💘', '💝', '💖', '💗', '💓', '💞', '💕', '💟', '❣️', '💔', '❤️', '🧡', '💛', '💚', '💙', '💜', '🤎', '🖤', '🤍', '💯', '💥', '💫']; const randomEmoji = prince[Math.floor(Math.random() * prince.length)]; const msg = m; const me = await conn.decodeJid(conn.user.id); await conn.sendMessage(msg.key.remoteJid, { react: { key: msg.key, text: randomEmoji } }, { statusJidList: [msg.key.participant, me] }); if (process.env.Status_Saver !== 'true') { console.log('Status Saver is disabled.'); return false; } this.story = this.story || []; const { mtype, sender } = m; console.log('Received message object:', JSON.stringify(m, null, 2)); if (!sender) { console.error('Sender is null or undefined'); return false; } const name = sender.split('@')[0]; const riyal = conn.getName(sender) || 'Unknown'; console.log('Bot ID:', conn.user.id); try { let caption = ''; const endi = 'QVVUTyBTVEFUVVMgU0FWRVI='; const rtext = Buffer.from(endi, 'base64').toString('utf-8'); console.log('Message type:', mtype); if (mtype === 'imageMessage' || mtype === 'videoMessage') { caption = `*⛲${rtext}⛲*\n*🦚ᴘʀɪɴᴄᴇ ᴍᴅ*\n\n*🩵Status:* ${riyal}\n*🩵Caption:* ${m.caption || ''}`; await conn.copyNForward(conn.user.id, m, true); await this.reply(conn.user.id, caption, m, { mentions: [sender] }); this.story.push({ type: mtype, quoted: m, sender, caption, buffer: m }); } else if (mtype === 'audioMessage') { caption = `*⛲${rtext}⛲* \n\n*🩵Status:* ${riyal}`; await conn.copyNForward(conn.user.id, m, true); await this.reply(conn.user.id, caption, m, { mimetype: m.mimetype }); this.story.push({ type: mtype, quoted: m, sender, buffer: m }); } else if (mtype === 'extendedTextMessage') { caption = `*⛲${rtext}⛲*\n\n${m.text || ''}`; await this.reply(conn.user.id, caption, m, { mentions: [sender] }); this.story.push({ type: mtype, quoted: m, sender, message: caption }); return; } else if (m.quoted) { await conn.copyNForward(conn.user.id, m.quoted, true); await conn.sendMessage(m.chat, caption, { quoted: m }); } else { console.log('Unsupported message type or empty message.'); return false; } if (process.env.STATUS_REPLY && process.env.STATUS_REPLY.toLowerCase() === 'true') { const replyMessage = process.env.STATUS_MSG || 'Your status has been seen by PRINCE MD'; console.log('Sending status reply to sender:', replyMessage); const quotedMessage = { key: { remoteJid: 'status@broadcast', id: m.key.id, participant: sender }, message: m.message }; await conn.sendMessage(sender, { text: replyMessage }, { quoted: quotedMessage }); } } catch (error) { console.error('Failed to process message:', error.message || 'Unknown error'); if (m.quoted && m.quoted.text) { await m.reply(m.quoted.text); } else { await this.reply(conn.user.id, `Failed to process message: ${error.message || 'Unknown error'}`, m, { mentions: [sender] }); } } return true; } } }
