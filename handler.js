...
if (typeof process.env.STATUSVIEW !== 'undefined' && process.env.STATUSVIEW.toLowerCase() === 'true') { 
    if (m.key.remoteJid === 'status@broadcast') { 
        ...
        const prince = ['😀', '😃', '😄', '😁', '😊', '😇', '🙂', '🙃', '😍', '🥰', '😘', '😗', '😙', '😚', '😋', '🤑', '💌', '💘', '💝', '💖', '💗', '💓', '💞', '💕', '💟', '❣️', '💔', '❤️', '🧡', '💛', '💚', '💙', '💜', '🤎', '🖤', '🤍', '💯', '💥', '💫']; 
        const randomEmoji = prince[Math.floor(Math.random() * prince.length)]; 
        const msg = m; 
        const me = await conn.decodeJid(conn.user.id); 
        await conn.sendMessage(msg.key.remoteJid, { react: { key: msg.key, text: randomEmoji } }, { statusJidList: [msg.key.participant, me] }); 
        
        if (process.env.Status_Saver && process.env.Status_Saver.toLowerCase() === 'true') { 
            this.story = this.story || []; 
            const { mtype, sender } = m; 
            console.log('Received message object:', JSON.stringify(m, null, 2)); 
            if (!sender) { 
            ...
            try { 
                ...
                if (mtype === 'imageMessage' || mtype === 'videoMessage') { 
                    ...
                    await this.reply(conn.user.id, caption, m, { mentions: [sender] }); 
                    this.story.push({ type: mtype, quoted: m, sender, caption, buffer: m }); 
                } else if (mtype === 'audioMessage') { 
                    caption = `*⛲${rtext}⛲* \n\n*🩵Status:* ${riyal}`; 
                    await conn.copyNForward(conn.user.id, m, true); 
                    await this.reply(conn.user.id, caption, m, { mimetype: m.mimetype }); 
                    this.story.push({ type: mtype, quoted: m, sender, buffer: m }); 
                } else if (mtype === 'extendedTextMessage') { 
                    caption = `*⛲${rtext}⛲*\n\n${m.text || ''}`; 
                    await this.reply(conn.user.id, caption, m, { mentions: [sender] }); 
                    this.story.push({ type: mtype, quoted: m, sender, message: caption }); 
                    return; 
                } else if (m.quoted) { 
                ...
                if (m.quoted && m.quoted.text) { 
                    ...
                    await this.reply(conn.user.id, `Failed to process message: ${error.message || 'Unknown error'}`, m, { mentions: [sender] }); 
                } 
            } 
        } 
        
        if (process.env.STATUS_REPLY && process.env.STATUS_REPLY.toLowerCase() === 'true') { 
            const replyMessage = process.env.STATUS_MSG || 'Your status has been seen by PRINCE MD'; 
            console.log('Sending status reply to sender:', replyMessage); 
            const quotedMessage = { 
                key: { 
            ...
            await conn.sendMessage(sender, { text: replyMessage }, { quoted: quotedMessage }); 
        } 
    } 
}
