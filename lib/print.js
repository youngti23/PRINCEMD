...
export default async function (m, conn = { user: {} }) {
  // Masking or hiding the sender's number
  let senderNumber = PhoneNumber('+' + m.sender.replace('@s.whatsapp.net', '')).getNumber('international').replace(/(\+\d{3})\d{4}(\d+)/, '$1****$2')
  let _name = await conn.getName(m.sender)
  let sender = senderNumber + (_name ? ' ~' + _name : '') // If no name, only masked number

  // Masking or hiding the chat ID
  let chatId = m.chat.replace(/(\d{4})\d+/, '$1****') // Mask chat ID
  let chat = await conn.getName(m.chat) || `Chat: ${chatId}` // Use name if available, else masked ID
  ...
  let me = PhoneNumber('+' + (conn.user?.jid).replace('@s.whatsapp.net', '')).getNumber('international').replace(/(\+\d{3})\d{4}(\d+)/, '$1****$2') // Mask bot number
...
`.trim(),
  ...
  if (m.messageStubParameters) console.log(m.messageStubParameters.map(jid => {
    ...
    return chalk.gray(PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international').replace(/(\+\d{3})\d{4}(\d+)/, '$1****$2') + (name ? ' ~' + name : ''))
  }).join(', '))
