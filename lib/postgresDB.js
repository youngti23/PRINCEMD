...
import url from 'url'; // Importing url module to parse the DATABASE_URL

const { Pool } = pkg; // Destructure Pool from pg

export class PostgresDB {
  constructor(url, options = {}) {
    const params = url.parse(url); // Parse URL to extract details like username, password, etc.
    const auth = params.auth.split(':'); // Split auth to get username and password
    ...
    this.pool = new Pool({
      ...
      host: params.hostname, // Host (e.g., localhost)
      port: params.port, // Port (e.g., 5432)
