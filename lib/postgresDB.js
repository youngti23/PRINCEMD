...
export class PostgresDB {
  constructor(url, options = {}) {
    ...
    this.pool = new Pool({
      ...
      ssl: {
        rejectUnauthorized: false, // If using self-signed certificates or no certificates
  ...
  async connectWithRetry() {
    ...
    while (retries > 0) {
      try {
        ...
        if (retries === 0) {
          throw new Error(
            "PostgreSQL connection failed after multiple attempts."
          );
        }
  ...
  async read() {
    ...
    try {
      ...
      console.log("Data loaded from PostgreSQL."); // Avoid printing the full data
    } catch (err) {
  ...
  async write(data) {
    ...
    try {
      ...
      // Store keys for logging after processing
      const keys = [];

      // Loop through each key-value pair and insert or update
      for (const [key, value] of Object.entries(data)) {
        ...
        keys.push(key); // Track keys being written
      }

      // Log the keys written after the loop
      console.log(`Data saved to PostgreSQL for keys: ${keys.join(", ")}`);
